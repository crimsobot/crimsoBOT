...
def time_convert(dateframe_col, timeframe_col, lat, lon, modify_date=False):
    ...
    for ii in range(len(dateframe_col)):
        ...
        time = datetime(year, swap(month), int(day), int(hh), int(mm), int(ss))
        # convert to local time
        time_loc = swaptz(time, lat, lon)
        # create new datestring and timestring, pop back in
        datestring = '%02d %s' % (time_loc.day, swap(time_loc.month))
    ...
    if source == 'n2yo':
        ...
        try:
            for event in pass_info['passes']:
                ...
                pass_list.append('{s} rises in {startAzCompass} · {p} peaks in {maxAzCompass}({maxEl}°) · {e} sets in {endAzCompass} · Mag: {pl}{mag}'.format(s=start, p=peak, e=end, pl=plus, **event))
        except:
            pass
...
def whereis(*arg):
    ...
    # now the URL!
    url = 'https://www.mapquestapi.com/staticmap/v5/map?locations={},{}&boundingbox={},{},{},{}&type=dark&size=800,600@2x&key={}'.format(lat,lon,bound[0],bound[2],bound[1],bound[3],MAPQUEST_API_KEY)
    # return url
    # bit.ly shortner
