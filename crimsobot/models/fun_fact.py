...
class FunFact(Model):
    ...
    @classmethod
    async def create_fact(cls, creator: DiscordUser, guild: int, subject: str, body: str) -> Tuple['FunFact', bool]:
        user = await User.get_by_discord_user(creator)

        # get queryset to check if fact exists
        all_facts = await FunFact.filter(subject=subject, body=body, guild_id=guild)

        if len(all_facts) == 0:
            fact = await FunFact.create(created_by=user, subject=subject, guild_id=guild, body=body)  # type: FunFact
            fact_created = True
        else:
            fact = all_facts[0]
            fact_created = False

        return fact, fact_created
